<?php
    include '../connect.php';
    session_start();
    $sql_show_prd = "SELECT * FROM product ORDER BY prod_count DESC";
                        ...
                        $i = 1;
                            while($row = mysqli_fetch_assoc($query_show_prd)){ ?>
                                    <tr style="text-align: center;">
                                        ...
                                        <td><?php echo $row['prod_detail']?></td>
                                        <td><?php echo number_format($row['prod_price'], 0, ',', '.')?></td>
                                        <td><?php echo $row['prod_origin'] ?></td>
                                        <td><img src="../assets/images/<?php echo $row['prod_image']?>"> </td>
                                        <td><?php echo $row['prod_quantity'] ?></td>
                                        <td><a href="update_product.php ?id=<?php echo $row['prod_id'] ?>"><button
                                                type="button" class="btn btn-sm btn-primary btn-create">Cập nhật
                                                </button></a></td>
                                        <td><a href="delete_product.php ?id=<?php echo $row['prod_id'] ?>"><button
